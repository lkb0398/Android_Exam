...
package com.lkb0398nate.androidexam.thread;
...
public class ThreadActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    // 백 그라운드 처리는 바로바로 보이지만, UI 변경은 쓰레드 종료시에 보여진다.
    private void runOnUiThreadExam() {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < 10; i++) {
                    try {
                        Thread.sleep(1000); // 스레드가 잠시 1초 쉰다.
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                        // 에러처리
                    }
    ...
    private void threadAndHandle() {
        // Handler 클래스 상속을 샹략 한 것.
    ...
    private class DownloadTask extends AsyncTask<Void, Integer, Void> {
        ...
        // UI Thread
        // doInBackground 에서 publicProcess 로 호출하면 호출 됨
        // 직접 호출 하지 않는 이유 : 죽으니까.
        @Override
        ...
        // donInBackgroun 에서 return 된 값이 파라메터로 넘어옴.
        @Override
